 ...
 * Layout: 
 * - Unified Scrolling (No inner scrollbars).
 * - Desktop: Split Screen (Visuals Left | Info Right).
 * - Mobile: Vertical Stack.
 * - 360 Viewer: Taller and drag-safe.
 * Font: Kantumruy Pro
 */
...
const ThreeSixtySection = ({ isDesktop = false }) => {
    ...
    return (
        <div className={`px-4 md:px-6 py-6 bg-zinc-900 ${isDesktop ? 'bg-transparent h-auto' : 'border-t border-neutral-800'} space-y-4`}>
            ...
            {/* Height: Mobile 500px, Desktop 600px */}
            <div className={`relative w-full rounded-xl overflow-hidden border border-neutral-700 group shadow-lg ${isDesktop ? 'h-[600px]' : 'h-[500px]'}`}>
                {start360 ? (
  ...
  return (
    <div className="font-kantumruy text-white w-full overflow-x-hidden bg-neutral-950">
      ...
      {!isOpened ? (
        ...
        // Removed h-[90vh] so the card can grow. Used min-h for full coverage.
        <div className="min-h-screen w-full bg-neutral-950 flex flex-col items-center justify-start lg:justify-center pt-0 lg:pt-10 pb-0 lg:pb-10 px-0 lg:px-4 animate-fade-in-up">
          
          {/* Main Card Container - Unified Scrolling */}
          <div className="w-full md:max-w-3xl lg:max-w-7xl mx-auto bg-zinc-900 lg:border lg:border-neutral-800 lg:shadow-[0_0_60px_rgba(185,28,28,0.3)] lg:rounded-3xl overflow-hidden relative flex flex-col lg:flex-row pb-20">
            
            {/* Top Decorative Line */}
            <div className="h-1 w-full bg-gradient-to-r from-red-700 via-red-500 to-amber-500 shrink-0 lg:hidden"></div>

            {/* --- LEFT COLUMN --- */}
            <div className="flex w-full lg:w-[55%] flex-col bg-black/20 relative lg:border-r border-neutral-800 shrink-0">
                <HeroSection isMobile={false} />
                <ThreeSixtySection isDesktop={true} />
            </div>

            {/* --- RIGHT COLUMN --- */}
            <div className="w-full lg:w-[45%] flex flex-col bg-zinc-900 relative">
                ...
                {/* RSVP FORM Section */}
                <div className="px-4 md:px-6 pt-6 bg-zinc-900 border-t border-neutral-800 lg:border-none mb-12">
            ...
            <div className="fixed bottom-0 w-full bg-black/95 backdrop-blur-lg p-4 flex flex-col gap-3 border-t border-neutral-800 z-50 lg:rounded-br-3xl lg:absolute lg:w-full">
